...
namespace PoliceSMS.Web.BaseWcf
{
    // 注意: 使用“重构”菜单上的“重命名”命令，可以同时更改代码、svc 和配置文件中的类名“SexService”。
    ...
    public class SexService : ISexService
    {
        ...
        public virtual string SaveOrUpdate(string json)
        {
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                ...
                Sex entity = JsonSerializerHelper.JsonToEntity<Sex>(json);
        ...
        public virtual string GetByHql(string hql)
        {
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                try
                {
                    Sex entity = sess.CreateQuery(hql).UniqueResult<Sex>();
        ...
        public virtual string GetListByHQL(string hql)
        {
            ...
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                try
                {
                    IList<Sex> ls = sess.CreateQuery(hql).List<Sex>();
        ...
        public virtual string GetListByHQLWithPaging(string condition)
        {
            ...
            try
            {
                ...
                //设置分页
                IList<Sex> ls = q.SetFirstResult(qc.FirstResult).SetMaxResults(qc.MaxResults).List<Sex>();
                //IList<T> ls = q.List<T>();
