    ...
    public class OrganizationService : IOrganizationService
    {
        ...
        public virtual string DeleteById(int id)
        {
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                ...
                try
                {
                    Organization entity = sess.Load<Organization>(id);
                    sess.Delete(entity);
                    tx.Commit();
        ...
        public virtual string GetByHql(string hql)
        {
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                try
                {
                    Organization entity = sess.CreateQuery(hql).UniqueResult<Organization>();
        ...
        public virtual string GetListByHQL(string hql)
        {
            ...
            using (ISession sess = HbmSessionFactory.OpenSession())
            {
                try
                {
                    IList<Organization> ls = sess.CreateQuery(hql).List<Organization>();
        ...
        public virtual string GetListByHQLWithPaging(string condition)
        {
            ...
            try
            {
                ...
                //设置分页
                IList<Organization> ls = q.SetFirstResult(qc.FirstResult).SetMaxResults(qc.MaxResults).List<Organization>();
                //IList<T> ls = q.List<T>();
